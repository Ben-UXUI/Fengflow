  ...
  return (
    <div className="p-4 rounded-2xl border border-[var(--border-dark)] bg-white shadow-[1px_1px_0px_0px_var(--accent)]">
      <div className="flex items-start justify-between mb-2">
        <div>
          <h4 className="font-display font-bold text-[var(--text-primary)]">{zone}</h4>
          <p className="font-sans text-sm text-[var(--text-muted)]">{lifeArea}</p>
        </div>
        <Badge variant={getStatusVariant()}>
          {status}
        </Badge>
      </div>
      <div className="font-sans text-xs text-[var(--text-secondary)] mb-2">
        Element: <span className="font-medium">{element}</span>
      </div>
      <p className="font-sans text-sm text-[var(--text-secondary)]">{note}</p>
    </div>
  )
}
